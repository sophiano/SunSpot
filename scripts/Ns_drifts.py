...
and the support vector machine on the number of sunspots (Ns) smoothed on a year. 
...
### standardisation
dataNs.standardisation(K=6200) #standardisation of the data
dataIC = dataNs.dataIC  #IC data without deviations
data = dataNs.data #data (IC and OC) with deviations
...
n_obs_IC = len(dataIC[~np.isnan(dataIC)])*100 / len(data[~np.isnan(data)])#27%
...
delta_min = chart.shift_size(data, pool, dataIC=dataIC,
                ...
                qt=0.5, missing_values ='reset')[1] #1,6 

delta_min = 1.6 #fixed value for reproduciability purpose

control_limit = chart.limit_CUSUM(dataIC, delta=delta_min, ARL0_threshold=200,
                block_length=bb_length, missing_values='reset') #19.35-20.625
...
### save models 
# filename = 'svr_Ns_365.sav'
# pickle.dump(reg, open(filename, 'wb'))
# filename = 'svc_Ns_365s.sav'
# pickle.dump(clf, open(filename, 'wb'))

### or load the models previously trained
reg = pickle.load(open('svm_models/svr_Ns_365.sav', 'rb'))
clf = pickle.load(open('svm_models/svc_Ns_365.sav', 'rb'))
...
for i in range(stat, stat+1): 
    ...
    #fig.savefig('figures/NS_drifts_%s.pdf' %station_names[i]) #save figures
