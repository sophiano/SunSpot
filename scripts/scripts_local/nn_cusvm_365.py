...
on unstandardised data smoothed on a year. 
...
with open('../../data/Nc_365', 'rb') as fichier:
      ...
      mu2_wht_levels = my_depickler.load() #mu2 with levels ('mu2')
...
std_cusvm = np.nanstd(dataIC_cusvm)
...
reg.load_weights("../../nn_models/nn_regression_365.h5")
...
# load weights into new model
reg_rnn.load_weights("../../nn_models/rnn_regression_365.h5")
...
h_loop = np.zeros((n))
for i in range(n): 
    h_loop[i] = nn_limit.cutoff(dataIC_nn, reg, ARL0_threshold=200, L_plus=0.25, 
...
# h_loop = np.zeros((n))
# for i in range(n): 
#     h_loop[i] = nn_limit.cutoff(dataIC_nn, reg, ARL0_threshold=200, L_plus=0.1, 
#                   n=2000, nmc=2000, block_length=wdw_length, 
...
h_loop = np.zeros((n))
for i in range(n): 
    h_loop[i] = nn_limit.cutoff(dataIC_cusvm, reg_adp, ARL0_threshold=200, L_plus=20, 
                  n=2000, nmc=2000, block_length=wdw_length, 
                  BB_method='MBB', chart='cusum') 
...
# h_loop = np.zeros((n))
# for i in range(n): 
#     h_loop[i] = nn_limit.rnn_cutoff(dataIC_nn, reg_rnn, ARL0_threshold=200, L_plus=0.1, 
#                   n=2000, nmc=2000, block_length=wdw_length, 
#                   BB_method='MBB', chart='cusum') 
...
h_loop = np.zeros((n))
for i in range(n): 
    h_loop[i] = nn_limit.rnn_cutoff(dataIC_cusvm, reg_rnn_adp, L_plus=20, 
                  n=2000, nmc=2000, block_length=wdw_length, 
                  BB_method='MBB', chart='cusum') 
...
h_cusvm = 33.2187 #16.9375 if delta=1.4


### unstandardised 
# h_loop = np.zeros((n))
# for i in range(n): 
#     h_loop[i] = chart.limit_CUSUM(dataIC_nn, delta=0.1, L_plus=2, 
#                     missing_values='omit', block_length=wdw_length, 
#                     BB_method='MBB') 

# h_cusvm = np.mean(h_loop) #0.0037 pour delta=0.1, pour delta=0.05 1.14
# h_cusvm = 0.0059

# h_loop = np.zeros((n))
# for i in range(n): 
#     h_loop[i] = chart.limit_CUSUM(dataIC_nn, delta=0.05, L_plus=2, 
#                     missing_values='omit', block_length=wdw_length, 
#                     BB_method='MBB') 
...
form = 'drift' #shape of the simulated shifts
...
for i in range(n_delt): 
    ...
    perfs[i,0] = nn_limit.ARL1_NN(dataIC_nn, reg, h_nn, delta=delt[i]/21, 
                      nmc=2000, n=2000, form = form, 
                      block_length=100, wdw_length=wdw_length) 
    
    ### Feed-forward nn with adaptive CUSUM
    # perfs[i,1] = nn_limit.ARL1_NN(dataIC_nn, reg, h_nn_adp, delta=delt[i]/21, 
    #                   nmc=2000, n=2000, form = form, 
    #                   block_length=100, wdw_length=wdw_length,
    #                   chart='cusum') 
    ...
    #               n_L=n_L, delta=delt[i]/21, form=form,
    #               nmc=2000, n=2000, block_length=wdw_length)
        
    
    ### rnn with cut-off 
    perfs[i,2] = nn_limit.ARL1_RNN(dataIC_nn, reg_rnn, h_rnn,  delta=delt[i]/21,
                    form=form, nmc=2000, n=2000, 
                    block_length=100, wdw_length=wdw_length)
    
    
    ### rnn with adaptive CUSUM
    # perfs[i,3] = nn_limit.ARL1_RNN(dataIC_nn, reg_rnn, h_rnn_adp,  delta=delt[i]/21,
    #                 form=form, nmc=2000, n=2000, 
    #                 block_length=100, wdw_length=wdw_length ,
    #                 chart='cusum')
    ...
    perfs[i,4] = chart.ARL1_CUSUM(dataIC_cusvm, h_cusvm, delta=delt[i], k=0.5,
                  block_length=100, form=form, 
                    nmc=2000, n=2000, BB_method='MBB')
    
    
    # perfs[i,4] = chart.ARL1_CUSUM(dataIC_nn, h_cusvm, delta=delt[i]/21, 
    #             k=0.025, block_length=100, form=form, 
    #                 nmc=2000, n=2000, BB_method='MBB')


    ### 4 classes method 
    perfs[i,5] = nn_limit.ARL1_4class(dataIC_nn, clf4, delta=delt[i]/21,
...
plt.ylabel('ARL1')
plt.show()
#fig.savefig('ARL1_drift_all.pdf') 


#save into txt files
# f = open('perfs_oscill_365.txt','w')
# for i in range(n_delt):
#     for j in range(4):
#         if j < 3:
