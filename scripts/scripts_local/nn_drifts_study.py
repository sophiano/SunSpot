...
The comparison works on the composite (Nc=Ns+10Ng) smoothed on 365 days.
...
# load weights into new model
clf.load_weights("nn_models/nn_classification_365.h5")
...
# load weights into new model
clf_rnn.load_weights("nn_models/rnn_classification_365.h5")
...
### train a regressor
regressor, scores = nn_train.feed_forward_4reg(dataIC, block_length)

### train a classifier
classifier, scores, matrix = nn_train.feed_forward_4class(dataIC, 
                                                          block_length)
...
for b in range(0, n_test-2, 3):
    ...
    for shape in range(3):
        ...
        X_test[b] = boot
        Y_test[b] = shift
        Y_perst[b] = boot[0] #first value of the window
        b += 1
...
# # fit scaler on data
# input_scaler.fit(X_test)
# # apply transform
# normalized = input_scaler.transform(X_test)
# # inverse transform
# inverse = input_scaler.inverse_transform(normalized)

# output_scaler = MinMaxScaler(feature_range=(-1,1))
# Y_test = Y_test.reshape(-1,1)
# output_scaler.fit(Y_test)
# normalized = output_scaler.transform(Y_test)
# inverse = output_scaler.inverse_transform(normalized)
