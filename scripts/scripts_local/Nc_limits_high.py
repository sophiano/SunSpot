...
and the support vector machine on the composite (Nc=Ns+10Ng) smoothed on eleven
years. 

This analysis aims at identifying an upper value in time above which the 
monitoring method would be unvalid/inaproppriate. 
Smoothing the data on 11 years still appears to work.
...
mu2 = err.long_term_error(Ns, period_rescaling=8, wdw=4000) #11 years
...
dataNs.level_removal(wdw=10000)  #remove intrisic levels
...
### standardisation
dataNs.standardisation(K=5000) #standardisation of the data
dataIC = dataNs.dataIC  #IC data without deviations
data = dataNs.data #data (IC and OC) with deviations
...
n_obs_IC = len(dataIC[~np.isnan(dataIC)])*100 / len(data[~np.isnan(data)])#31%
...
#large range
#bb_length = bbl.block_length_choice(data, bbl_min=10, bbl_max=110, bbl_step=10) #60
#smaller range
#bb_length = bbl.block_length_choice(dataIC, bbl_min=, bbl_max=64, bbl_step=2) #52
bb_length = 60
...
#                 qt=0.5, missing_values ='reset')[1] #do not work, too much corr.

delta_min = 1.5

control_limit = chart.limit_CUSUM(dataIC, delta=delta_min, ARL0_threshold=200,
                block_length=bb_length, missing_values='reset') #9.8 (5-15)
control_limit = 10
