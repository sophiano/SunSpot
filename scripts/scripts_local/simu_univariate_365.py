# -*- coding: utf-8 -*-
"""
Created on Wed Sep  1 14:50:39 2021
...
separately. This analysis is done for data smoothed on 365 days. 
...
stat = [i for i in range(len(station_names)) if station_names[i] == 'KZ'][0]
...
blocks_IC = np.array(blocks_IC).reshape(-1,1)
        

n = 5
h_loop = np.zeros((n))
for i in range(n):  
    h_loop[i] = chart.limit_CUSUM(blocks_IC, delta=delta_min, ARL0_threshold=200,
        L_plus=40, block_length=bb_length, missing_values='reset')

h_ind = np.mean(h_loop) 

h_ind = 0 #18.68 SM out of control and 0 for FU 
...
for i in range(n_delt):
    ...
    perfs[i, 0] = chart.ARL1_CUSUM(blocks_IC, h, delta=delt[i], k=0.7,
                  block_length=bb_length, form=form, 
                    nmc=2000, n=2000, BB_method='MBB', missing_values='reset')
    
    perfs[i, 1] = chart.ARL1_CUSUM(blocks_IC, h_large, delta=delt[i], k=0.7,
                  block_length=bb_length, form=form, 
                    nmc=2000, n=2000, BB_method='MBB', missing_values='reset')
    
    perfs[i, 2] = chart.ARL1_CUSUM(blocks_IC, h_ind, delta=delt[i], k=0.7,
                  block_length=bb_length, form=form, 
                    nmc=2000, n=2000, BB_method='MBB', missing_values='reset')
