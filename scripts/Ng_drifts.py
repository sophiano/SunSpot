...
and the support vector machine on the number of groups (Ng) smoothed on a year. 
...
mu2 = err.long_term_error(Ng, period_rescaling=14, wdw=365)
...
### standardisation
dataNs.standardisation(K=3200) #standardisation of the data
dataIC = dataNs.dataIC  #IC data without deviations
data = dataNs.data #data (IC and OC) with deviations
...
n_obs_IC = len(dataIC[~np.isnan(dataIC)])*100 / len(data[~np.isnan(data)])#16%
...
control_limit = chart.limit_CUSUM(dataIC, delta=delta_min, ARL0_threshold=200,
                block_length=bb_length, missing_values='reset') #14.5

control_limit = 15 #fixed value
...
### save models 
# filename = 'svr_Ng_365.sav'
# pickle.dump(reg, open(filename, 'wb'))
# filename = 'svc_Ng_365.sav'
# pickle.dump(clf, open(filename, 'wb'))

### or load the models previously trained
#reg = pickle.load(open('svm_models/svr_Ng_365.sav', 'rb'))
#clf = pickle.load(open('svm_models/svc_Ng_365.sav', 'rb'))
...
for i in range(stat, stat+1): 
    ...
    #fig.savefig('figures_Ng/drifts_%s.pdf' %station_names[i]) #save figures
