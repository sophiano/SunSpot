...
with open(data_path + '/Nc_27', 'rb') as file:  
...
m =  nn_train.input_in_autocorr(dataIC, 0.2) #close to 50
...
# with open("../nn_models/rnn_regression_27.json", "w") as json_file:
#     json_file.write(model_json)
# ### serialize weights to HDF5
# regressor.save_weights("../nn_models/rnn_regression_27.h5")
...
# with open("../nn_models/rnn_classification_27_test.json", "w") as json_file:
#     json_file.write(model_json)
# ### serialize weights to HDF5
# classifier.save_weights("../nn_models/rnn_classification_27_test.h5")
...
regressor.load_weights("nn_models/rnn_regression_27.h5")
...
blocks[block_length-1:,:] = bb.MBB(data[:,stat].reshape(-1,1), block_length, NaN=True, all_NaN=False) 
...
h = nn_control.rnn_cutoff(dataIC, regressor, L_plus=1, nmc=2000, 
                n=2000, block_length=block_length, BB_method='MBB')#0.384765625

h = 0.389 #fixed value (reproduciability)
...
plt.axis([time[start], time[stop], -1.4, 1.4])
