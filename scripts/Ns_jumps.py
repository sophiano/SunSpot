...
and the support vector machine on the number of sunspots (Ns) smoothed on 
27 days. 
...
### standardisation
dataNs.standardisation(K=2800) #standardisation of the data
dataIC = dataNs.dataIC  #IC data without deviations
data = dataNs.data #data (IC and OC) with deviations
...
delta_min = chart.shift_size(data, pool, dataIC=dataIC,
                ...
                qt=0.5, missing_values ='reset')[1] #1.5 

delta_min = 1.5 #fixed value for reproduciability purpose

control_limit = chart.limit_CUSUM(dataIC, delta=delta_min, ARL0_threshold=200,
                block_length=bb_length, missing_values='reset') #12.77-13
...
control_limit_mv = chart.limit_CUSUM(dataIC, delta=delta_min, ARL0_threshold=ARL0,
              block_length=bb_length, missing_values='omit') #14.41-14.17-14.17
...
for i in range(stat, stat+1): 
    ...
    #fig.savefig('figures/NS_jumps_%s.pdf' %station_names[i]) #save figures
