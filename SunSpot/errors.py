...
def long_term_minima(data, period_rescaling, wdw=81, min_perc_per_wdw=10,
                    center=True):
    """ 
    This function computes the long-term error (mu2(i,t)) with also values at 
    solar minima to ensure that the monitoring makes sense when Mt=0. 
    ...
    for i in range(n_obs):
        for j in range(n_stations):
            if not np.isnan(Mt[i]) and Mt[i] > 0:
                ratio[i,j] = data[i,j]/Mt[i] #Yi(t)/Mt
