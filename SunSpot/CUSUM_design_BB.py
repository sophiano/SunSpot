...
def search_CUSUM_MV(data, delta=1.5, k=None, ARL0_threshold=200, rho=2, L_plus=20, 
                    ...
                    missing_values='omit', gap=0, block_length=None):
    """ 
...
def reccurentDesign_CUSUM(data, indexIC, dataIC=None, delta=1.5, k=None, 
    ...
    while (np.abs(delta - delta_prev) > accuracy):
        L = search_CUSUM_MV(dataIC, delta=delta, k=k, L_plus=L_up, 
                            ARL0_threshold=ARL0_threshold, block_length=block_length, 
                            missing_values=missing_values, gap=gap, verbose=False)
        delta_prev = delta
...
if __name__ == "__main__":            
    ...
    t3 = search_CUSUM_MV(data, delta=1, ARL0_treshold=465, verbose=True, block_length=1)#5
