...
def alerts_info(data, L_plus, delta, wdw_length, clf, reg, L_minus = None,
    ...
    cut : float, optional 
        ...
        negative) chart statistics are constrained to be equal to and 
        lower than 'cut' (resp. equal to or superior than -'cut').
        When None, cut is equal to '2 * L_plus'. The default is None.
...
def plot_monitoring(data, level, L_plus, time, form_plus, form_minus, size_plus, 
                      size_minus, C_plus, C_minus, name, L_minus=None, 
                      time_start=None, time_stop=None):
    """
    Draws the main quantities of the monitoring. 
    
    This function plots different quantities that resume the main features 
    of the monitoring into four panels. 
    The first panel represents the data with levels,
    the second shows the standardized residuals (without levels), 
    the third panel displays the CUSUM statistics applied to the residuals
    ...
    data : 1D-array
        A (single) series of standardized observations to be monitored. 
    level : 1D-array
        The original series (with level and without standardization).
    L_plus : float 
        Value for the positive control limit.
    time : 1D-array
    ...
    f2.set_ylim([-10,10]); f2.set_xlim([time[start], time[stop]])
    plt.xticks(x_ticks)
    ...
    f3.set_ylim([-15,15]); f3.set_xlim([time[start], time[stop]])
    plt.xticks(x_ticks)
    ...
    f4.set_ylim([-10,10]); f4.set_xlim([time[start], time[stop]])
    plt.xticks(x_ticks)
    plt.xlabel('year')
    plt.tick_params(axis='x', which='major') 
    #plt.grid(axis='x', color='r', linewidth=0.5)
...
def plot_zoom(data, time, form_plus, form_minus, size_plus, 
             size_minus, name, time_start=None, time_stop=None):
    """
    Plots with the predictions (forms and sizes) of the support 
    vector machine (SVM).
    ...
    f1.set_ylim([-10,10]); f1.set_xlim([time[start], time[stop]])
    plt.xticks(x_ticks)
    plt.xlabel('year')
    plt.tick_params(axis='x', which='major') 
    #plt.grid(axis='x', color='r', linewidth=0.5)
...
def plot_3panel(data, L_plus, time, form_plus, form_minus, size_plus, 
                      size_minus, C_plus, C_minus, name, L_minus=None, 
                      time_start=None, time_stop=None):
    """
    Draws the main quantities of the monitoring. 
    ...
    The first panel represents the standardized residuals (without levels)
    the second panel displays the CUSUM statistics applied to the residuals
    in square-root scale
    and the last panel shows the predicted shift sizes and forms.
    ...
    plt.plot(time[start:stop], data[start:stop])
    plt.plot([time[start], time[stop]], [0, 0], 'k-', lw=2)
    f1.set_ylim([-8.5,8.5]); f1.set_xlim([time[start], time[stop]])
    plt.xticks(x_ticks)
    f1.axes.get_xaxis().set_ticklabels([]) 
    ...
    plt.plot([time[start], time[stop]], [-np.sqrt(abs(L_minus)), -np.sqrt(abs(L_minus))], 'k-', lw=2.2)
    plt.legend(loc='upper right', ncol=2, fontsize=10)
    f2.set_ylim([-15,15]); f2.set_xlim([time[start], time[stop]])
    plt.xticks(x_ticks)
    f2.axes.get_xaxis().set_ticklabels([]) 
    ...
    f3.set_ylim([-6,6]); f3.set_xlim([time[start], time[stop]])
    plt.xticks(x_ticks)
    plt.xlabel('year')
    plt.tick_params(axis='x', which='major') 
    #plt.grid(axis='x', color='r', linewidth=0.5)
