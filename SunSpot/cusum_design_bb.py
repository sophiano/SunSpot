...
def ARL0_CUSUM_MV(data, L_plus, L_minus=None, delta=1.5, k=None, nmc=4000,
    ...
    """ 
    Computes the in-control (IC) average run length (ARL0) of the CUSUM chart
    ...
    missing_values : str, optional
        Strings that indicates how to deal with the missing values (MV). 
        The string value should be chosen among: 'omit', 'reset' and 'filled',
        where:
        ...
        a specified gap length. The chart statistics is simply propagated 
        through smaller gaps. Default is 'omit'.
    gap :  int >= 0 , optional
        The length of the gaps above which the chart statistics are reset,
        expressed in days. Default is zero. 
    ...
    assert missing_values in ['filled', 'reset', 'omit'], "Undefined value for 'missing_values'"
...
def search_CUSUM_MV(data, delta=1.5, k=None, ARL0_threshold=200, rho=2, L_plus=60, 
   ...
   at the desired accuracy.
    ...
    missing_values : str, optional
        Strings that indicates how to deal with the missing values (MV). 
        The string value should be chosen among: 'omit', 'reset' and 'filled', where
        'omit' removes the blocks containing MV ;
        'filled' fills-up the MV by the mean of each series ;
        'reset' resets the chart statistics at zero for gaps larger than
        a specified gap length. The chart statistics is simply propagated 
...
def ARL1_CUSUM_MV(data, L_plus, L_minus=None, form='jump', delta=1.5, k=None, 
    ...
    for b in range(nmc):
        ...
        if form == 'oscillation':
            ...
            boot = shift/(n)*(np.arange(n)**power) + boot
            pass
        else: 
            boot = boot + shift
            pass
...
def ARL_values_MV(data, L_plus, L_minus=None, form='jump', delta=1.5, k=None,
    ...
    for b in range(nmc):
        ...
        if form == 'oscillation':
            ...
            power = np.random.uniform(1.5,2)
            boot[n_shift:] = shift/(n_shift)*(np.arange(n_shift)**power) + boot[n_shift:]
            pass
        else: 
...
def shift_sizes(data, L_plus, L_minus=None, delta=1.5, k=None, nmc=4000, 
...
def reccurentDesign_CUSUM(data, indexIC, dataIC=None, delta=1.5, k=None, 
    ...
    """ 
    Recurrent algorithm that computes the control limits of the chart and 
    a target shift size. 
    ...
    dataIC : 2D-array, optional
         ...
         If None, the IC dataset is directly retrieved from the 'data'.
    delta : float >= 0, optional
    ...
    while (np.abs(delta - delta_prev) > accuracy):
        L = search_CUSUM_MV(dataIC, delta=delta, k=k, L_plus=L_plus, ARL0_threshold=ARL0_threshold, 
                            block_length=block_length, missing_values=missing_values,
                            gap=gap, verbose=verbose, BB_method=BB_method)
        delta_prev = delta
        shifts = shift_sizes(dataOC, L_plus=L, delta=delta, k=k, 
...
if __name__ == "__main__":            
    ...
    shifts = shift_sizes(data, L1, block_length=1) 
