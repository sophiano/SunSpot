...
def standardisation(data, dataIC, K):
    """ 
    Standardizes the input data with time-dependent parameters. 
    
    The time-varying mean and the variance are computed across the IC series 
    and along a small window of time using K nearest neighbors (KNN) estimators.
...
def choice_K(data, dataIC, plot=True, start=200, stop=10000, step=200):
    ...
    data : 2D-array
        A matrix representing a panel of time-series to be standardised
        by K-NN estimators. (rows: time, columns: series)
    dataIC : 2D-array
        A subset of the panel containing stable time-series to be standardised.
        The time-varying mean and variance are estimated on this stable subset. 
    plot : bool, optional 
    ...
    if plot: 
        ...
        print('K where mean is closer to 0: ', x[np.argmin(abs(mean_data-0))])
        print('K where std is closer to 1: ', x[np.argmin(abs(std_data-1))])
...
if __name__ == "__main__":
    ...
    data = dataNs.data
    K_knee = choice_K(dataIC, dataIC) #2200
        
    scale = K_knee/len(pool) #7
    dataNs.standardisation(K=2200)
    dataIC = dataNs.dataIC
    data = dataNs.data
    ...
    dataIC = dataNs.dataIC
    data = dataNs.data 
    K_knee = choice_K(dataIC, dataIC) #4400 or 3800 if IC poool
    
    #standardisation
    scale = K_knee/len(pool) #25~26
    dataNs.standardisation(K=4400)
    dataIC = dataNs.dataIC
    data = dataNs.data
