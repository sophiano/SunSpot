...
if __name__ == "__main__":
    ...
    ## export data 
    # with open('data_1981', 'wb') as file: 
    #      my_depickler = pickle.Unpickler(file)
    #      Ns = my_depickler.load() #number of spots
    #      Ng = my_depickler.load() #number of sunspot groups
    #      Nc = my_depickler.load() #Ns+10Ng
    ...
    data = dataNs.data 
    K_knee = choice_K(data, dataIC) #2800
        
    scale = K_knee/len(pool) #7
    dataNs.standardisation(K=K_knee)
    dataIC = dataNs.dataIC
    data = dataNs.data
    ...
    dataIC = dataNs.dataIC
    data = dataNs.data 
    K_knee = choice_K(data, dataIC) #6200
